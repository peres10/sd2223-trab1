...
class DiscoveryImpl implements Discovery {
	...
	static final InetSocketAddress DISCOVERY_ADDR = new InetSocketAddress("226.226.226.226", 9000);
	...
	private static Map<String, Set<URI>> storedAnnoucments = new ConcurrentHashMap<>();
	...
	public URI[] knownUrisOf(String serviceName, int minEntries) {
		...
		while(totalReplies < minEntries){
			...
			if(storedAnnoucments.containsKey(serviceName)){
				knownURIs.addAll(storedAnnoucments.get(serviceName));
				if (knownURIs.size() >= minEntries){
					break;
				}
			} else {
	...
	private void startListener() {
		...
		if(storedAnnoucments == null){
			storedAnnoucments = new HashMap<>();
		}
		new Thread(() -> {
				...
				for (;;) {
					try {
						...
						if (parts.length == 2) {
							...
							//URI[] uris = {uri};
							storedAnnoucments.computeIfAbsent(serviceName, (k) -> ConcurrentHashMap.newKeySet()).add( uri );
						}
